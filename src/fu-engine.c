			  ...
			  GError **error)
{
	...
	if (fu_device_has_internal_flag(device, FU_DEVICE_INTERNAL_FLAG_SAVE_INTO_BACKUP_REMOTE)) {
		g_autoptr(GBytes) blob_cab =
		    fu_bytes_get_contents_stream_full(stream, 0x0, G_MAXUINT32, error);
		if (blob_cab == NULL)
			return FALSE;
	...
	if ((flags & FWUPD_INSTALL_FLAG_OFFLINE) > 0 && !fu_engine_is_running_offline(self)) {
		...
		blob_cab = fu_bytes_get_contents_stream_full(stream, 0x0, G_MAXUINT32, error);
		if (blob_cab == NULL)
			return FALSE;
...
#else
		{
			g_autoptr(GBytes) blob_cab2 = NULL;
			blob_cab2 =
			    fu_bytes_get_contents_stream_full(stream, 0x0, G_MAXUINT32, error);
			if (blob_cab2 == NULL)
				return FALSE;
			passim_item_set_bytes(passim_item, blob_cab2);
		}
#endif
