...
static gboolean
fu_igsc_code_firmware_parse_stream(FuFirmware *firmware,
				   GInputStream *stream,
				   gsize offset,
				   ...
				   GError **error)
{
	...
	g_autoptr(FuFirmware) fw_imgi = NULL;
	g_autoptr(FuFirmware) fw_info = NULL;
	...
	/* FuIfwiFptFirmware->parse_stream */
	if (!FU_FIRMWARE_CLASS(fu_igsc_code_firmware_parent_class)
		 ->parse_stream(firmware, stream, offset, flags, error))
		return FALSE;

	fw_info =
	    fu_firmware_get_image_by_idx(FU_FIRMWARE(self), FU_IFWI_FPT_FIRMWARE_IDX_INFO, error);
	if (fw_info == NULL)
		return FALSE;
	stream_info = fu_firmware_get_stream(fw_info, error);
	if (stream_info == NULL)
		return FALSE;
	...
	fw_imgi =
	    fu_firmware_get_image_by_idx(FU_FIRMWARE(self), FU_IFWI_FPT_FIRMWARE_IDX_IMGI, error);
	if (fw_imgi == NULL)
		return FALSE;
	stream_imgi = fu_firmware_get_stream(fw_imgi, error);
	if (stream_imgi == NULL)
		return FALSE;
...
fu_igsc_code_firmware_class_init(FuIgscCodeFirmwareClass *klass)
{
	...
	klass_firmware->parse_stream = fu_igsc_code_firmware_parse_stream;
