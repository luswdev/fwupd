				       ...
				       GError **error)
{
	...
	/* get cmbd block info */
	cmdb_tmp = fu_input_stream_read_buf(stream,
...
static gboolean
fu_kinetic_dp_puma_firmware_parse_stream(FuFirmware *firmware,
					 GInputStream *stream,
					 gsize offset,
					 ...
					 GError **error)
{
	...
	isp_drv_stream = fu_partial_input_stream_new(stream, HEADER_LEN_ISP_DRV_SIZE, isp_drv_size);
...
fu_kinetic_dp_puma_firmware_class_init(FuKineticDpPumaFirmwareClass *klass)
{
	...
	klass_firmware->parse_stream = fu_kinetic_dp_puma_firmware_parse_stream;
