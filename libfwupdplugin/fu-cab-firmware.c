			   ...
			   GError **error)
{
	...
	if (helper->compression == FU_CAB_COMPRESSION_MSZIP) {
		...
		bytes_comp = fu_bytes_get_contents_stream_full(helper->stream,
							       *offset + hdr_sz,
							       blob_comp,
							       error);
		if (bytes_comp == NULL)
...
static gboolean
fu_cab_firmware_parse_stream(FuFirmware *firmware,
			     GInputStream *stream,
			     gsize offset,
...
fu_cab_firmware_class_init(FuCabFirmwareClass *klass)
{
	...
	klass_firmware->parse_stream = fu_cab_firmware_parse_stream;
