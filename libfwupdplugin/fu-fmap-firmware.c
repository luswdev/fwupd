...
static gboolean
fu_fmap_firmware_parse_stream(FuFirmware *firmware,
			      GInputStream *stream,
			      gsize offset,
			      ...
			      GError **error)
{
	...
	/* subclassed */
	if (klass_firmware->parse_stream != NULL) {
		if (!klass_firmware->parse_stream(firmware, stream, offset, flags, error))
			return FALSE;
	}
...
fu_fmap_firmware_class_init(FuFmapFirmwareClass *klass)
{
	...
	klass_firmware->parse_stream = fu_fmap_firmware_parse_stream;
