...
static gboolean
fu_efi_firmware_volume_parse_stream(FuFirmware *firmware,
				    GInputStream *stream,
				    gsize offset,
				    ...
				    GError **error)
{
	...
	if ((flags & FWUPD_INSTALL_FLAG_IGNORE_CHECKSUM) == 0) {
		...
		hdr_blob = fu_input_stream_read_bytes(stream, 0x0, hdr_length, error);
		if (hdr_blob == NULL)
			return FALSE;
		checksum_verify = fu_sum16w_bytes(hdr_blob, G_LITTLE_ENDIAN);
...
fu_efi_firmware_volume_class_init(FuEfiFirmwareVolumeClass *klass)
{
	...
	klass_firmware->parse_stream = fu_efi_firmware_volume_parse_stream;
